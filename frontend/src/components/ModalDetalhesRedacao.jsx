...
export default function ModalDetalhesRedacao({ redacao, onClose, onUpdated }) {
  ...
  const siseduDiscursivaMap = [
    ...
    { key: 'argumentacao', title: 'Consistência & Progressão' },
    { key: 'repertorio', title: 'Produtividade de Repertório' }
  ];

  const siseduEticoCriticaMap = [
    { key: 'direitos_humanos', title: 'Respeito aos Direitos Humanos' },
    { key: 'justificativa_critica', title: 'Consciência Crítica & Justificação' },
    { key: 'eficacia_proposta', title: 'Eficácia da Intervenção Cidadã' }
  ];
              ...
              <div className="space-y-3.5">
                {devolutivaEnem && (
                  <div className="bg-[#1f8a65]/10 border border-[#1f8a65]/30 rounded-xl p-4 sm:p-5 space-y-2.5 shadow-xs animate-fadeIn">
                    <div className="flex items-center gap-2 text-[#1f8a65] font-bold text-xs uppercase tracking-wider pb-1 border-b border-[#1f8a65]/20">
              ...
              <div className="space-y-4">
                {(devolutivaSisedu || Object.values(siseduDescritores).some(d => d?.nivel === 'Inicial')) && (
                  <div className="bg-[#f54e00]/10 border border-[#f54e00]/30 rounded-xl p-4 sm:p-5 space-y-2.5 shadow-xs animate-fadeIn">
                    <div className="flex items-center gap-2 text-[#f54e00] font-bold text-xs uppercase tracking-wider pb-1 border-b border-[#f54e00]/20">
                      <AlertTriangle className="w-4 h-4 text-[#f54e00] shrink-0" />
                      <span>Plano de Intervenção Pedagógica — Matriz SISEDU / SPAECE</span>
                    </div>
                    <FormattedFeedbackText 
                      text={devolutivaSisedu || "Atenção: O estudante apresentou descritores em Nível Inicial. Recomenda-se aplicar atividade direcionada de reescrita com foco em contra-argumentação (D15) e coesão lexical (D12)."} 
                    />
                  </div>
                )}
                ...
                {/* SEÇÃO QUALITATIVA: DIMENSÕES DISCURSIVA & ÉTICO-CRÍTICA (PROJETO ÁGORA) */}
                {/* SEÇÃO QUALITATIVA: DIMENSÕES DISCURSIVA & ÉTICO-CRÍTICA (PROJETO ÁGORA) */}
                {(() => {
                  const dClarezaNivel = sisedu.dimensao_discursiva?.clareza_tese?.nivel || siseduDescritores['D06']?.nivel || siseduDescritores['D13']?.nivel || (c3Val >= 160 && c2Val >= 160 ? 'Avançado' : (c3Val >= 120 ? 'Adequado' : 'Inicial'));
                  ...
                  const dProgNivel = sisedu.dimensao_discursiva?.argumentacao?.nivel || siseduDescritores['D16']?.nivel || siseduDescritores['D14']?.nivel || (c3Val >= 160 ? 'Avançado' : (c3Val >= 120 ? 'Adequado' : 'Inicial'));
                  ...
                  const dRepNivel = sisedu.dimensao_discursiva?.repertorio?.nivel || siseduDescritores['D05']?.nivel || (c2Val >= 160 ? 'Avançado' : (c2Val >= 120 ? 'Adequado' : 'Inicial'));
                  ...
                  return (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-3 border-t border-[#e6e5e0]">
                      
                      {/* CARD 1: DIMENSÃO DISCURSIVA */}
                      <div className="bg-[#ffffff] border border-[#26251e] rounded-xl p-4 sm:p-5 shadow-xs space-y-3.5">
                      ...
                      {/* CARD 2: DIMENSÃO ÉTICO-CRÍTICA (IDÊNTICO À IMAGEM) */}
                      <div className="bg-[#ffffff] border border-[#26251e] rounded-xl p-4 sm:p-5 shadow-xs space-y-3.5">
                        <h4 className="text-xs font-bold text-[#26251e] uppercase tracking-wider font-mono border-b border-[#e6e5e0] pb-2">
